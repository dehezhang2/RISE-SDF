...
class ColmapDatasetBase():
    ...
    def setup(self, config, split):
        ...
        self.split = split
        self.rank = get_rank()
        self.hold_every = self.config.get("hold_every", 8)
        ...
        if self.split == 'predict':
            self.all_c2w = create_spheric_poses(self.all_c2w[:,:,3], n_steps=self.config.n_test_traj_steps).to(self.rank)
            ...
            train_indices = [i for i in range(len(imdata.values())) if i % self.hold_every != 0]
            self.all_c2w = all_c2w[train_indices, ...].float().to(self.rank)
            self.all_images, self.all_fg_masks = torch.stack(self.all_images, dim=0).float()[train_indices, ...], torch.stack(self.all_fg_masks, dim=0).float()[train_indices, ...]
        elif self.split == 'test':
            test_indices = [i for i in range(len(imdata.values())) if i % self.hold_every == 0]
            self.all_c2w = all_c2w[test_indices, ...].float().to(self.rank)
            self.all_images, self.all_fg_masks = torch.stack(self.all_images, dim=0).float()[test_indices, ...], torch.stack(self.all_fg_masks, dim=0).float()[test_indices, ...]
        ...
        print(f"Using split {self.split}, length {self.all_images.shape[0]}")
...
class ColmapDataModule(pl.LightningDataModule):
    ...
    def val_dataloader(self):
        return self.general_loader(self.val_dataset, batch_size=1, shuffle=True)
