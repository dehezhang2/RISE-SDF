...
class NeuSSystem(BaseSystem):
    ...
    def preprocess_data(self, batch, stage):
        ...
        if stage in ['train']:
            c2w = self.dataset.all_c2w[index.cpu()]
            x = torch.randint(
            ...
            rays_o, rays_d = get_rays(directions, c2w, opencv_format=self.dataset.config.get("opencv_format"))
            rgb = self.dataset.all_images[index, y, x].view(-1, self.dataset.all_images.shape[-1]).to(self.rank)
            fg_mask = self.dataset.all_fg_masks[index, y, x].view(-1).to(self.rank)
        else:
            c2w = self.dataset.all_c2w[index.cpu()][0]
            if self.dataset.directions.ndim == 3: # (H, W, 3)
                directions = self.dataset.directions
            elif self.dataset.directions.ndim == 4: # (N, H, W, 3)
                directions = self.dataset.directions[index.cpu()][0] 
            rays_o, rays_d = get_rays(directions, c2w, opencv_format=self.dataset.config.get("opencv_format"))
            rgb = self.dataset.all_images[index.cpu()].view(-1, self.dataset.all_images.shape[-1]).to(self.rank)
            fg_mask = self.dataset.all_fg_masks[index.cpu()].view(-1).to(self.rank)

        rays = torch.cat([rays_o, F.normalize(rays_d, p=2, dim=-1)], dim=-1).to(self.rank)
