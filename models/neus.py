...
from lib.nerfacc import ContractionType, OccupancyGrid, ray_marching
...
class NeuSModel(BaseModel):
    def setup(self):
        ...
        if self.config.grid_prune:
            ...
            if self.config.learned_background:
                # Fall back to nerfacc==0.3.5 for ray_marching in contracted space
    ...
    def forward_bg_(self, rays):
        ...
        def sigma_fn(t_starts, t_ends, ray_indices):
            ...
            density = self.geometry_bg(positions)
            return density[..., None]

        _, t_max, hits = ray_aabb_intersect(rays_o, rays_d, self.scene_aabb.reshape(1, 6))
        # if the ray intersects with the bounding box, start from the farther intersection point
        hit_mask = hits.squeeze(-1)
        rays_o[hit_mask] = rays_o[hit_mask] + t_max[hit_mask] * rays_d[hit_mask]

        with torch.no_grad():
            # Call ray_marching for nerfacc==0.3.5
            ...
            # For compatibility with nerfacc>=0.5.0
            t_starts.squeeze_(-1)
            t_ends.squeeze_(-1)
        ...
        density, feature = self.geometry_bg(positions, with_feature=True)
    ...
    def forward_(self, rays):
        ...
        if t_starts.shape[0] == 0:
            alpha = torch.empty((0,), device=t_starts.device)
            midpoints = torch.empty((0,1), device=t_starts.device)
            rgb = torch.empty((0,3), device=t_starts.device)
            normal = torch.empty((0,3), device=t_starts.device) 
            sdf = torch.empty((0,), device=t_starts.device) 
            sdf_grad = torch.empty((0,), device=t_starts.device) 
            dists = torch.empty((0,), device=t_starts.device) 
        else:
            ray_indices = ray_indices.long()
            t_origins = rays_o[ray_indices]
            t_dirs = rays_d[ray_indices]
