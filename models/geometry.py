...
class VolumeDensity(BaseImplicitGeometry):
            ...
            with torch.set_grad_enabled(self.training or with_grad):
                ...
                if with_grad:
                    d_output = torch.ones_like(
                        density, requires_grad=False, device=density.device
                    )
    ...
    def regularizations(self, out):
        reg_dict = {}
        if hasattr(self.encoding_with_network, 'regularizations'):
            for k, v in self.encoding_with_network.regularizations().items():
                reg_dict[k + '_density'] = v
                ...
                if with_grad:
                    if self.grad_type == 'analytic':
                        ...
                        if with_laplace:
                            ...
                            if hasattr(self, 'center') and hasattr(self, 'scale'):
                                points_d = (points_d_ - self.center) / self.scale + 0.5
                            else:
                                points_d = contract_to_unisphere(points_d_, self.radius, self.contraction_type) # points normalized to (0, 1)
